...
template <typename T>
T run(std::string_view input, auto filter) {
    auto iter = input
        | std::views::split(',')
        | std::views::transform([](auto subrange) { return std::string_view(subrange); })
        | std::views::transform([filter](auto sv) {
            auto range = sv
                | std::views::split('-')
                | std::views::transform([](auto subrange) {return std::string_view(subrange);})
...
#define RUN(name, input) std::cout << STR(name) ": " << run<uint64_t>(input, name##_filter) << "\n"

    auto part1_filter = [](auto str) {
        if (str.size() % 2 != 0)
            return false;
        return str.substr(0, str.size() / 2) == str.substr(str.size() / 2, str.size());
    };

    auto part2_filter = [](auto str) {
        for (auto i : std::views::iota(1, static_cast<int>(str.size() / 2) + 1)) {
            if ([](auto part, auto str) {
                    if ((str.size() % part.size()) != 0)
                        return false;
    ...
    RUN(part1, test_input);
    RUN(part1, input);
    RUN(part2, test_input);
    RUN(part2, input);

    // std::cout << "part1 test: " << run<uint64_t>(test_input, part1_filter) << "\n";
    // std::cout << "part1: " << run<uint64_t>(input, part1_filter) << "\n";
    // std::cout << "part2 test: " << run<uint64_t>(test_input, PART2_FILTER) << "\n";
    // std::cout << "part2: " << run<uint64_t>(input, PART2_FILTER) << "\n";
}
